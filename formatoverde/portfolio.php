...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "editarvisibilidade")) {
  $updateSQL = sprintf("UPDATE portfolio SET visivel=%s WHERE cod=%s",
...
mysql_select_db($database_formatoverde, $formatoverde);
$query_portfolio = "SELECT * FROM portfolio ORDER BY `data` DESC";
$query_limit_portfolio = sprintf("%s LIMIT %d, %d", $query_portfolio, $startRow_portfolio, $maxRows_portfolio);
$portfolio = mysql_query($query_limit_portfolio, $formatoverde) or die(mysql_error());
$row_portfolio = mysql_fetch_assoc($portfolio);

if (isset($_GET['totalRows_portfolio'])) {
  $totalRows_portfolio = $_GET['totalRows_portfolio'];
} else {
  $all_portfolio = mysql_query($query_portfolio);
  $totalRows_portfolio = mysql_num_rows($all_portfolio);
}
...
<div id="main_container">
    ...
    <div class="center_content">  
     ...
     <div class="left_content">
      ...
      <?php if ($pageNum_portfolio > 0) { // Show if not first page ?>
  <a href="<?php printf("%s?pageNum_portfolio=%d%s", $currentPage, max(0, $pageNum_portfolio - 1), $queryString_portfolio); ?>"><< prev</a>
  <?php } // Show if not first page ?>
  <?php if ($pageNum_portfolio == 0) { // Show if first page ?>
    <span class="disabled"><< prev</span>
    <?php } // Show if first page ?>
    
    <?php 
	     for($i = 0; $i < $totalPages_portfolio + 1; $i++) {
			 if($pageNum_portfolio==($i)){?>
				 <span class="current"><?php echo $i+1; ?> </span>
				 <?php
				 }
			 else{
             echo "<a href='portfolio.php?pageNum_portfolio=$i'>".($i+1)."</a>";
			 }
        }
	?>    
    
<?php if ($pageNum_portfolio < $totalPages_portfolio) { // Show if not last page ?>
        <a href="<?php printf("%s?pageNum_portfolio=%d%s", $currentPage, min($totalPages_portfolio, $pageNum_portfolio + 1), $queryString_portfolio); ?>">next >></a>
        <?php } // Show if not last page ?>
        <?php if ($pageNum_portfolio >= $totalPages_portfolio) { // Show if last page ?>
  <span class="disabled">next >></span>
  <?php } // Show if last page ?>
           </div>
       </td>
    <td width="150"><a href="adprojeto.php" class="menuitem">Adicionar Projeto</a></td>
					   </tr>
					</table>
    ...
    <thead>
    	<tr>
          ...
          <th width="250" class="rounded" scope="col">Área</th>
          <th width="250" class="rounded" scope="col">Cliente</th>
        </tr>
    </thead>
        <tfoot>
    	<tr>
    ...
    <tr>
    <td>
    <img src="../portfolio/<?php echo $row_portfolio['cod']; ?>/main.jpg" width="200"/>
    
    
        <form action="<?php echo $editFormAction; ?>" method="POST" name="editarvisibilidade" enctype="multipart/form-data" >
     <input  id="visibilidade" name="visibilidade" type="hidden" value="<?php if($row_portfolio['visivel']==1){echo 0;} else{echo 1;}?>"/>
    <input type="image" src="images/<?php if($row_portfolio['visivel']==1){echo visivel;} else{echo invisivel;}?>.png" alt="Submit Form" class="visibilidade"/>
    <input name="cod" type="hidden" value="<?php echo $row_portfolio['cod']; ?>"/>
         <input type="hidden" name="MM_insert" value="editarvisibilidade" />
         <input type="hidden" name="MM_update" value="editarvisibilidade" />
    </form>
    
    
    <a href="editarprojeto.php?cod=<?php echo $row_portfolio['cod']; ?>"><img src="images/user_edit.png" alt="" title="" border="0" /></a><a  href="eliminarprojeto.php?cod=<?php echo $row_portfolio['cod']; ?>"class="ask"><img src="images/trash.png" alt="" title="" border="0" /></a></td>
    
    <td><?php echo $row_portfolio['titulo'];?></td>
    <td><?php echo $row_portfolio['area']; ?></td>
    <td><?php echo $row_portfolio['cliente']; ?></td>
    </tr>
