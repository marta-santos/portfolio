...
if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "adbanner")) {
  $insertSQL = sprintf("INSERT INTO etiqueta (titulo, data, texto, titulofr, datafr, textofr, tituloes, dataes, textoes, titulopt, datapt, textopt, diainicio, mesinicio, anoinicio, diafim, mesfim, anofim, datatxt, imagem, link) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)",
...
$target_path = $target_path.sprintf('%03d', mysql_insert_id()).'.jpg';
...
<script>
  $(function() {
	...
	$('#datatxt').val($('#diafim').val().$('#mesfim').val().$('#anofim').val());
  });
...
function validar() {
  ...
  return true;
  $('#datatxt').attr('value', $('#diafim').val().$('#mesfim').val().$('#anofim').val())
}
...
<div id="main_container">
    ...
    <div class="left_content">
        ...
        <h1>Adicionar Banner</h1>
         ...
         <div class="form">
         <form action="<?php echo $editFormAction; ?>" method="POST" name="adbanner" enctype="multipart/form-data" onsubmit="return validar();">
         
         <table width="625" border="0">
  <tr>
    <td> 
    <h2>Data início:</h2>
             <input type="text" class="datepicker" name="datainicio" id="inicio">
            <input name="diainicio" id="diainicio" class="dia" type="hidden" value="" />
            <input name="mesinicio" id="mesinicio" class="mes" type="hidden" value="" />
            <input name="anoinicio" id="anoinicio" class="ano" type="hidden" value="" /></td>
    <td>       
    <h2>Data fim:</h2>
               <input type="text" class="datepicker" name="datafim" id="fim">
            <input name="diafim" id="diafim" class="dia" type="hidden" value="" />
            <input name="mesfim" id="mesfim" class="mes" type="hidden" value="" />
            <input name="anofim" id="anofim" class="ano" type="hidden" value="" /></td>
  </tr>
</table>
  				<p>Imagem:</p> <input name="imagem" type="file" class="file" onchange="readURL(this);"/>
                <img id="blah" src="" />
                (imagem .jpg)
            	<p>Link:</p>
         		<textarea name="link" cols="80"> </textarea>
         		<input name="datatxt" cols="80" id="datatxt" type="hidden"> </input>  
                <div class="sep"></div>
			<h2>Inglês</h2>
         		<p>Título:</p>
         		<textarea name="titulo" cols="80"> </textarea>    
		 		<p>Data:</p>
		 		<textarea name="data" cols="80"> </textarea>
   				<p>Texto:</p>
			...
			<h2>Português</h2>
        		...
        		<p>Data:</p>
		 		<textarea name="datapt" cols="80"> </textarea>
   				<p>Texto:</p>
            ...
            <h2>Espanhol</h2>
        		...
        		<p>Data:</p>
		 		<textarea name="dataes" cols="80"> </textarea>
   				<p>Texto:</p>
            ...
            <h2>Francês</h2>
        		...
        		<p>Data:</p>
		 		<textarea name="datafr" cols="80"> </textarea>
   				<p>Texto:</p>
         ...
         <input type="hidden" name="MM_insert" value="adbanner" />
         </form>
         </div>       
